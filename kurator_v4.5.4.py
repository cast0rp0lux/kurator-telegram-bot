...
# ─── Version ──────────────────────────────────────────────────────────────────
BOT_VERSION = "Kurator 📀 Music Discovery Engine (v4.5.4)"
...
def _filter_artists_by_decade(artists, decades, message=None):
    """
    Filter artists by era using Last.fm tags + listener stats.
    ...
    with ThreadPoolExecutor(max_workers=12) as ex:
        futures = {ex.submit(_artist_decade_from_lastfm, a): a for a in artists}
        for f in as_completed(futures):
            artist = futures[f]
            try:
                artist_decades = f.result()
                if not artist_decades:
                    unknown.append(artist)  # no decade info — keep as fallback
                elif artist_decades & decades:
                    passed.append(artist)   # intersection with selected decades
                # else: artist is from wrong era — filtered out
            except Exception:
                unknown.append(artist)

    log.info(f"Era filter: {len(artists)} total → {len(passed)} passed → {len(unknown)} unknown")

    # If we have enough — return passed only
    if len(passed) >= 20:
        return passed

    # If too few, supplement with unknowns
    if len(passed) + len(unknown) >= 10:
        log.info("Supplementing with unknown-era artists")
        return passed + unknown[:max(0, 30 - len(passed))]

    # Complete fallback — not enough data at all
    log.info("Era filter returned too few — using full artist pool")
    return artists
