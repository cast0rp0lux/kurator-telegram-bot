...
def select_tracks_with_decades(artists, size=None, decades=None, message=None, mode="playlist", style_filter=None, use_mb=False, genre=None):
    ...
    # Expand pool using genre graph if too small — conservative thresholds
    if genre and len(pool) < 30:
        log.info(f"Pool too small ({len(pool)}) — expanding genre '{genre}' to near relatives")
        near_artists = []
        for related in _expand_genre(genre, "near"):
            if len(near_artists) >= len(pool):  # max 50% from near
                break
            styles = _resolve_genre_styles(related, decades)
            extra  = _get_era_artists_from_discogs(decades, mode=mode,
                                                    max_artists=20,
                                                    style_override=styles)
            near_artists.extend([a for a in extra if a not in pool])
            log.info(f"  +{len(extra)} from near '{related}'")
        pool.extend(near_artists)
        log.info(f"Pool after near expansion: {len(pool)}")

    if genre and len(pool) < 15:
        log.info(f"Pool still small ({len(pool)}) — expanding to mid relatives")
        mid_artists = []
        for related in _expand_genre(genre, "mid"):
            if len(mid_artists) >= len(pool) // 2:  # max 33% from mid
                break
            styles = _resolve_genre_styles(related, decades)
            extra  = _get_era_artists_from_discogs(decades, mode=mode,
                                                    max_artists=15,
                                                    style_override=styles)
            mid_artists.extend([a for a in extra if a not in pool])
            log.info(f"  +{len(extra)} from mid '{related}'")
        pool.extend(mid_artists)
        log.info(f"Pool after mid expansion: {len(pool)}")
