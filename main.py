...
def _get_era_artists_from_lastfm(genre, decades, max_artists=200):
    """
    Get artists for a genre from Last.fm tag.gettopartists.
    Then filter by era using MB album years.
    Much better coverage than Discogs for specific genre tags.
    """
    ...
    names = [a["name"] for a in items if a.get("name")]
    log.info(f"Last.fm tag '{genre}': {len(names)} artists")

    if not decades:
        random.shuffle(names)
        return names[:max_artists]

    # Filter by era using MB album years — stop early when enough found
    lo = min(DECADE_YEARS[d][0] for d in decades)
    hi = max(DECADE_YEARS[d][1] for d in decades)

    passed  = []
    unknown = []
    TARGET  = min(max_artists, 80)  # stop when we have enough
    ...
    log.info(f"Last.fm era filter: {len(passed)} passed, {len(unknown)} unknown for {decades}")

    # If not enough verified, supplement with unknowns
    if len(passed) < 20:
        passed += unknown[:max(0, 30 - len(passed))]
...
def handle_buttons(update, context):
    ...
    # ── decade_confirm: execute pending generation ────────────────────────────
        ...
        if gen_action == "playlist":
            ...
            # Verify names against Last.fm
            verified = []
            with ThreadPoolExecutor(max_workers=12) as ex:
                def _check(name):
                    try:
                        d = lastfm("artist.getinfo", artist=name)
                        n = d.get("artist", {}).get("name", "")
                        return n if n else None
                    except Exception:
                        return None
                futures = {ex.submit(_check, a): a for a in pool}
                for f in as_completed(futures):
                    try:
                        r = f.result()
                        if r:
