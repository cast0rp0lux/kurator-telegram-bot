...
def _get_era_artists_from_lastfm(genre, decades, max_artists=150):
    ...
    Returns shuffled list capped at max_artists.
    Era filtering happens downstream in _fetch_track_from_early_albums.
    """
    ...
    names = [a["name"] for a in items
             if a.get("name") and int(a.get("listeners", 0) or 0) >= 1000]
    log.info(f"Last.fm tag '{genre}': {len(names)} artists (filtered by listeners)")
...
def _fetch_track_for_genre(artist, decade_year_range):
    """
    Fetch a track for Free Explore genre playlists.
    MB is optional — if no era album found, fallback to _fetch_top_track immediately.
    No retries, no delays beyond the global MB semaphore.
    """
    try:
        mbid, _ = _mb_find_artist(artist)
        if mbid:
            ...
            if albums and decade_year_range:
                ...
                era_albums = [a for a in albums if a.get("year") and lo <= int(a["year"]) <= hi]
                if not era_albums:
                    # MB found albums but none in era — artist is wrong era, discard
                    return None
