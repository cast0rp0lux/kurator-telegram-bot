...
# ─── Version ──────────────────────────────────────────────────────────────────
BOT_VERSION = "Kurator 📀 Music Discovery Engine (v4.5.0)"
...
TRACK_LINKS_PAGE    = 10
MB_USER_AGENT       = "Kurator/4.5.0 (telegram bot)"
...
def _track_year_from_lastfm(artist, track):
    """Try to get track year from Last.fm album info. Returns int or None."""
    try:
        data = lastfm("track.getInfo", artist=artist, track=track)
        album = data.get("track", {}).get("album", {})
        # Try release date from track info
        for attr in ["@attr", "releasedate"]:
            rd = album.get(attr, {})
            if isinstance(rd, str) and len(rd) >= 4:
                year = rd.strip()[:4]
                if year.isdigit():
                    return int(year)
        # Try wiki published date
...
def _in_decades(year, decades):
    """Check if a year falls within any of the selected decades."""
    if not decades or not year:
        return True
    for d in decades:
        lo, hi = DECADE_YEARS.get(d, (0, 9999))
        if lo <= year <= hi:
            return True
    return False
...
def select_tracks_with_decades(artists, size=None, decades=None, chat_id=None):
    """Select tracks filtered by decade if specified."""
    if not decades:
    ...
    keys_added = set()
    random.shuffle(artists)
    candidates = artists[:target * 10]  # larger pool to compensate for filtering
        ...
        for f in as_completed(futures):
            ...
            try:
                ...
                if result:
                    artist, track_name, key = result
                    if not track_in_history(key) and key not in keys_added:
                        # Check year — fetch from Last.fm
                        year = _track_year_from_lastfm(artist, track_name)
                        if _in_decades(year, decades):
                            tracks.append(f"{artist} - {track_name}")
                            keys_added.add(key)
            except Exception as e:
                log.error(f"select_tracks_with_decades: {e}")

    # If too few tracks found with filter, fill with unfiltered tracks
    if len(tracks) < target * 0.4:
        log.info(f"Decade filter returned only {len(tracks)} tracks, filling without filter")
        extra = select_tracks([a for a in artists if a not in [t.split(" - ")[0] for t in tracks]],
                              size=target - len(tracks))
        tracks.extend(extra[:target - len(tracks)])
...
def _decade_selector_buttons(chat_id, confirm_cb, back_cb):
    """Build decade toggle buttons for a given chat_id."""
    selected = _pending_decades.get(chat_id, set())
    buttons  = []
    row      = []
    for d in DECADES:
        tick = "✅" if d in selected else "⬜"
        row.append(InlineKeyboardButton(f"{tick} {d}", callback_data=f"decade_toggle|{d}|{confirm_cb}|{back_cb}"))
        if len(row) == 4:
            buttons.append(row)
            row = []
    if row:
        buttons.append(row)
    label = f"✅ Generate — {_decade_label(chat_id)}" if selected else "✅ Generate (no era selected)"
    buttons.append([InlineKeyboardButton(label, callback_data=confirm_cb)])
    buttons.append([InlineKeyboardButton("⬅ Back", callback_data=back_cb)])
    return buttons

def _show_era_choice(query, chat_id, title, confirm_cb, back_cb):
    """Show Any era / Select decade choice before generating."""
    query.edit_message_text(
        f"{title}\n\nSelect era:",
        reply_markup=InlineKeyboardMarkup([
            [InlineKeyboardButton("🕰️ Any era",       callback_data=confirm_cb)],
            [InlineKeyboardButton("📅 Select decade", callback_data=f"decade_select|{confirm_cb}|{back_cb}")],
...
def send_playlist(message, tracks, title="✦ Kurator's Pick", branded=True, chat_id=None, size=None, map_chat_id=None, suppress_warning=False):
    target = size or PLAYLIST_SIZE
    if not tracks:
        message.reply_text(
...
def playlist(update, context):
    ...
    if context.args:
        ...
        sent, timer = _working_message(msg, "Still selecting…")
        result = select_tracks(expand_artist_graph(seeds))
        _cancel_working(sent, timer)
        send_playlist(msg, result, title="✦ Kurator's Pick", branded=True, chat_id=chat_id)

def dig(update, context):
    msg = update.message
    msg.reply_text("⛏️ Digging deeper…")
    seeds        = extract_seed_artists()
    sent, timer  = _working_message(msg, "Going deeper…")
    result       = select_tracks(expand_artist_graph_deep(seeds))
    _cancel_working(sent, timer)
    send_playlist(msg, result, title="✦ Kurator's Dig", branded=True, chat_id=update.effective_chat.id)

def rare(update, context):
    msg          = update.message
    msg.reply_text("💎 Searching for hidden gems…")
    sent, timer  = _working_message(msg, "Hunting for gems…")
    result       = select_tracks(expand_artist_graph_rare(extract_seed_artists()))
    _cancel_working(sent, timer)
    send_playlist(msg, result, title="✦ Kurator's Rare", branded=True, chat_id=update.effective_chat.id)
...
def _render_map(message, artist_query, chat_id):
    ...
    # Fetch artist info (MusicBrainz + Discogs + Last.fm)
    info = _get_artist_info(artist_query)
    if not info["genres"]:
        info["genres"] = [s for s, _ in sorted_styles[:4]]

    # Use official name going forward
...
def handle_buttons(update, context):
    ...
    if action == "cmd":
        ...
        if value == "menu":
            ...
            _show_era_choice(query, chat_id, "📀 Kurator's Pick", "gen|playlist", "cmd|menu")

        elif value == "dig":
            _pending_decades.pop(chat_id, None)
            _show_era_choice(query, chat_id, "⛏️ Kurator's Dig", "gen|dig", "cmd|menu")

        elif value == "rare":
            _pending_decades.pop(chat_id, None)
            _show_era_choice(query, chat_id, "💎 Kurator's Rare", "gen|rare", "cmd|menu")
    ...
    # ── decade_select: show decade toggle grid ────────────────────────────────
    elif action == "decade_select":
        sub        = value.split("|", 1)
        confirm_cb = sub[0]
        back_cb    = sub[1] if len(sub) > 1 else "cmd|menu"
        _pending_decades.setdefault(chat_id, set())
        query.edit_message_text(
            "📅 Select decades — tap to toggle:",
            reply_markup=InlineKeyboardMarkup(
                _decade_selector_buttons(chat_id, confirm_cb, back_cb)
            )
        )
    ...
    # ── decade_toggle: toggle a decade on/off ─────────────────────────────────
        ...
        sub        = value.split("|", 2)
        decade     = sub[0]
        confirm_cb = sub[1] if len(sub) > 1 else "cmd|menu"
        back_cb    = sub[2] if len(sub) > 2 else "cmd|menu"
    ...
    # ── gen: execute generation with optional decade filter ───────────────────
    elif action == "gen":
        ...
        if value == "playlist":
            ...
            result      = select_tracks_with_decades(expand_artist_graph(seeds), decades=decades)
            _cancel_working(sent, timer)
            era_tag = f" — {_decade_label_from_set(decades)}" if decades else ""
            send_playlist(message, result, title=f"✦ Kurator's Pick{era_tag}", branded=True, chat_id=chat_id)

        elif value == "dig":
            query.edit_message_text("⛏️ Digging deeper…")
            seeds       = extract_seed_artists()
            sent, timer = _working_message(message, "Going deeper…")
            result      = select_tracks_with_decades(expand_artist_graph_deep(seeds), decades=decades)
            _cancel_working(sent, timer)
            era_tag = f" — {_decade_label_from_set(decades)}" if decades else ""
            ...
            result      = select_tracks_with_decades(expand_artist_graph_rare(extract_seed_artists()), decades=decades)
            _cancel_working(sent, timer)
            era_tag = f" — {_decade_label_from_set(decades)}" if decades else ""
            send_playlist(message, result, title=f"✦ Kurator's Rare{era_tag}", branded=True, chat_id=chat_id)

        elif value.startswith("trail|"):
            sub    = value.split("|", 2)
            ...
            result = select_tracks_with_decades(names, decades=decades)
            _cancel_working(sent, timer)
            era_tag = f" — {_decade_label_from_set(decades)}" if decades else ""
            ...
            query.edit_message_text(f"🔖 Building {style.upper()} playlist…")
            data  = lastfm("tag.gettoptracks", tag=style, limit=100)
            items = data.get("tracks", {}).get("track", [])
            names = list({t["artist"]["name"] for t in items if t.get("artist")})
            random.shuffle(names)
            sent, timer = _working_message(message, "Still building…")
            result = select_tracks_with_decades(names, size=GENRE_PLAYLIST_SIZE, decades=decades)
            _cancel_working(sent, timer)
            era_tag = f" — {_decade_label_from_set(decades)}" if decades else ""
            send_playlist(message, result, title=f"🔖 {style.upper()}{era_tag}",
    ...
    # ── map_albums ────────────────────────────────────────────────────────────
        ...
        if not albums:
            query.answer("No studio albums found.", show_alert=True)
            return
        buttons = []
        for album in albums:
        ...
        buttons.append([InlineKeyboardButton(f"⬅ Back to {display_name[:20]}", callback_data=f"map_back|{chat_id}")])
        query.edit_message_text(
    ...
    # ── map_similar ───────────────────────────────────────────────────────────
        ...
        # Get similar artists list (1 hop)
        similar = [s["name"] for s in
                   lastfm("artist.getsimilar", artist=artist, limit=25)
                   .get("similarartists", {}).get("artist", [])]
    ...
    # ── explore_artist: navigate to a similar artist's map card ──────────────
        ...
        query.edit_message_text(f"🔭 Mapping {new_artist.upper()}…")
    ...
    # ── map_back ──────────────────────────────────────────────────────────────
        ...
        if history_stack:
            ...
            query.edit_message_text(f"🔭 Mapping {prev_artist.upper()}…")
    ...
    # ── build: playlist from style — era choice first ─────────────────────────
        ...
        style = value
        _pending_decades.pop(chat_id, None)
        _show_era_choice(query, chat_id, f"🔖 {style.upper()}", f"gen|build|{style}", f"map_back|{chat_id}")
