...
# ─── Version ──────────────────────────────────────────────────────────────────
BOT_VERSION = "Kurator 📀 Music Discovery Engine (v4.8.3)"
...
def _clean_track_title(title):
    ...
    noise_patterns = [
        ...
        r'\s*=\s*.+$',                          # = Translation (e.g. "Space Truckin' = Carga Espacial")
    ]
    cleaned = title
...
ERA_HAVE_RANGES = {
    "playlist": (50,  15000),  # known but not mega-hits
...
GENRE_TO_DISCOGS_STYLES = {
    "rock": {
        "50s": ["Rock & Roll", "Rockabilly"],
        "60s": ["Garage Rock", "Psychedelic Rock", "Surf", "Beat", "Freakbeat", "Acid Rock"],
        "70s": ["Hard Rock", "Prog Rock", "Glam", "Proto-Punk", "Space Rock", "Krautrock", "Stoner Rock"],
        "80s": ["Post-Punk", "Indie Rock", "New Wave", "Hardcore", "Noise Rock", "Jangle Pop"],
        "90s": ["Grunge", "Alternative Rock", "Britpop", "Stoner Rock", "Math Rock", "Shoegaze"],
        "00s": ["Post-Punk Revival", "Indie Rock", "Experimental Rock", "Post-Rock"],
        "10s": ["Indie Rock", "Post-Rock", "Drone", "Noise Rock"],
    },
    "pop": {
        "60s": ["Beat", "Sunshine Pop", "Baroque Pop", "Chamber Pop", "Merseybeat"],
        "70s": ["Glam", "Soft Rock", "Disco", "New Romantic"],
        "80s": ["Synth-Pop", "New Romantic", "New Wave", "Dream Pop", "Jangle Pop"],
        "90s": ["Dream Pop", "Indie Pop", "Britpop", "Noise Pop", "Chamber Pop"],
        "00s": ["Indie Pop", "Chamber Pop", "Neo-Psychedelia"],
        "10s": ["Synth-Pop", "Indie Pop", "Ambient Pop", "Chillwave", "Bedroom Pop"],
        "20s": ["Bedroom Pop", "Indie Pop", "Hyperpop"],
    },
    "jazz": {
        "50s": ["Cool Jazz", "Hard Bop", "Bebop"],
        "60s": ["Free Jazz", "Jazz-Rock", "Post Bop", "Soul Jazz"],
        "70s": ["Fusion", "Jazz-Rock", "Jazz-Funk"],
        "80s": ["Contemporary Jazz"],
        "90s": ["Future Jazz", "Acid Jazz"],
        "00s": ["Future Jazz", "Contemporary Jazz"],
    },
    "blues": {
        "50s": ["Jump Blues", "Rhythm & Blues", "Electric Blues"],
        "60s": ["British Blues", "Chicago Blues", "Blues Rock"],
        "70s": ["Blues Rock", "Electric Blues"],
    },
    "funk": {
        "60s": ["Soul", "Funk", "Rhythm & Blues"],
        "70s": ["Funk", "Soul", "Disco", "Jazz-Funk"],
        "80s": ["Funk", "Soul"],
    },
    "soul": {
        "60s": ["Soul", "Rhythm & Blues", "Northern Soul", "Gospel"],
        "70s": ["Soul", "Funk", "Disco"],
        "80s": ["Soul", "Funk"],
    },
    "electronic": {
        "70s": ["Krautrock", "Kosmische", "Minimal", "Ambient", "Experimental"],
        "80s": ["Synth-Pop", "EBM", "Industrial", "Minimal Wave", "New Wave", "Darkwave"],
        "90s": ["Trip-Hop", "Ambient Techno", "Drum n Bass", "Ambient"],
        "00s": ["Minimal", "Experimental"],
        "10s": ["Chillwave", "Witch House", "Drone", "Vaporwave"],
        "20s": ["Ambient", "Experimental"],
    },
    "metal": {
        "70s": ["Hard Rock", "Heavy Metal"],
        "80s": ["Thrash Metal", "Heavy Metal", "Hardcore"],
        "90s": ["Doom Metal", "Stoner Rock", "Noise Rock"],
        "00s": ["Post-Metal", "Drone Metal"],
    },
    "punk": {
        "70s": ["Proto-Punk", "Punk"],
        "80s": ["Hardcore", "Post-Punk", "No Wave", "Noise Rock"],
        "90s": ["Post-Hardcore", "Emo", "Hardcore"],
    },
    "folk": {
        "60s": ["Folk Rock", "Acoustic", "Baroque Pop"],
        "70s": ["Folk Rock", "Canterbury Scene"],
        "00s": ["Freak Folk", "New Weird America", "Indie Folk", "Free Folk"],
        "10s": ["Indie Folk", "Freak Folk"],
    },
    "country": {
        "50s": ["Country", "Western Swing", "Rockabilly"],
        "60s": ["Country", "Folk Rock"],
        "70s": ["Country Rock"],
    },
    "reggae": {
        "60s": ["Ska", "Rocksteady"],
...
def select_tracks_with_decades(artists, size=None, decades=None, message=None, mode="playlist", style_filter=None, use_mb=False, genre=None):
    ...
    # Optional MusicBrainz validation — check artist begin_year falls in selected decades
    if use_mb and verified and decades:
        ...
        with ThreadPoolExecutor(max_workers=6) as ex:
            futures = {ex.submit(_artist_decade_from_mb, a): a for a in verified}
            for f in as_completed(futures):
                artist = futures[f]
                try:
                    active = f.result()
...
def playlist(update, context):
    ...
    if context.args:
        ...
        if tag.lower().strip() in GENRES_NEED_ERA:
            # Generic genre — era is mandatory, no "Any era" option
            msg.reply_text(
                f"🎵🏷️ {tag.title()}\n\n"
...
def handle_buttons(update, context):
    ...
    # ── build: playlist from style — era choice first ─────────────────────────
        ...
        if style.lower().strip() in GENRES_NEED_ERA:
            # Generic genre — era mandatory, no "Any era" option
            ...
            # Specific genre — era is optional
