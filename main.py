...
# ─── Version ──────────────────────────────────────────────────────────────────
BOT_VERSION = "Kurator 📀 Music Discovery Engine (v4.6.1)"
...
def _render_map(message, artist_query, chat_id):
    ...
    # If no styles found, try MusicBrainz official name for Discogs retry
    if not counter:
        info_tmp      = _get_artist_info(artist_query)
        discogs_name  = info_tmp.get("official_name") or artist_query
        if discogs_name.lower() != artist_query.lower():
            try:
                data2 = requests.get(
                    "https://api.discogs.com/database/search",
