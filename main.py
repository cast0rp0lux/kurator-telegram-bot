...
# ─── Version ──────────────────────────────────────────────────────────────────
BOT_VERSION = "Kurator 📀 Music Discovery Engine (v4.8.4)"
...
def _build_genre_playlist(message, chat_id, style):
    ...
    # 70% obscure + 30% mainstream for balance
    random.shuffle(obscure)
    random.shuffle(mainstream)
    pool = obscure + mainstream[:max(5, GENRE_PLAYLIST_SIZE // 3)]
...
def _get_artist_info(artist_query):
    ...
    if mbid:
        time.sleep(1.1)  # respect MB 1 req/s limit
        ...
        for t in tags[:8]:
            tag_name = tag_index_key = t["name"].title()
            tag_index[tag_name] = tag_index.get(tag_name, 0) + t.get("count", 1)
        save_tag_index()

        time.sleep(1.1)  # second MB call
