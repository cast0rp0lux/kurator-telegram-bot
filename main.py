...
# ─── Version ──────────────────────────────────────────────────────────────────
BOT_VERSION = "Kurator 📀 Music Discovery Engine (v4.6.3)"
...
DECADE_TAGS = {
    "50s": ["1950s", "fifties", "50s"],
    "60s": ["1960s", "sixties", "60s"],
    "70s": ["1970s", "seventies", "70s"],
    "80s": ["1980s", "eighties", "80s"],
    "90s": ["1990s", "nineties", "90s"],
    "00s": ["2000s", "00s"],
    "10s": ["2010s", "10s"],
    "20s": ["2020s", "20s"],
}

def _get_era_seed_artists(decades, skip_top=45, fetch=150):
    """
    Get non-mainstream artists from the selected decades using Last.fm decade tags.
    Skips the top `skip_top` most popular artists to avoid mainstream.
    """
    artists = set()
    for decade in decades:
        tags = DECADE_TAGS.get(decade, [decade])
        for tag in tags[:2]:
            try:
                data  = lastfm("tag.gettopartists", tag=tag, limit=fetch)
                items = data.get("topartists", {}).get("artist", [])
                for a in items[skip_top:]:
                    name = a.get("name", "")
...
def _expand_era_pool(decades, depth=1):
    """
    Build an artist pool for era-based playlists.
    depth=0: direct tag artists (skip top 25)
    depth=1: similar artists of those (Dig)
    """
    seeds = _get_era_seed_artists(decades, skip_top=25, fetch=150)
    random.shuffle(seeds)

    if depth == 0:
        return seeds

    # Expand one level of similar artists
    pool = set()
    with ThreadPoolExecutor(max_workers=8) as ex:
        sample = seeds[:40]
    ...
    # Remove the seeds themselves and blacklisted artists
    pool -= set(seeds)
    pool = {a for a in pool if not _is_blacklisted(a)}
    return list(pool) if pool else seeds
...
def _filter_artists_by_decade(artists, decades, message=None):
    ...
    pool = artists[:60] if len(artists) > 60 else artists
    random.shuffle(pool)
    ...
    if len(passed) >= 10:
        return passed
    # Not enough verified artists — return what we have without unknown fallback
    log.info("Era filter: few verified artists — using passed only, no unknown fallback")
    return passed if passed else pool[:30]
...
def select_tracks_with_decades(artists, size=None, decades=None, message=None, mode="playlist"):
    """
    Select tracks filtered by era.
    When decades are selected, ignores the user history pool and builds
    an era-specific pool from Last.fm decade tags.
    mode: "playlist" (direct tag artists), "dig" (similar of tag artists), "rare" (obscure)
    """
    if not decades:
    ...
    # Build era-specific pool based on mode
    if mode == "rare":
        pool = _expand_era_pool(decades, depth=1)
        # Filter by listeners for rare
        if message:
            _safe_reply(message, "💎 Filtering for obscure artists…")
        rare_pool = []
        with ThreadPoolExecutor(max_workers=10) as ex:
            futures = {ex.submit(_fetch_listeners, a): a for a in pool[:200]}
            for f in as_completed(futures):
                try:
                    artist, listeners = f.result()
                    if 0 < listeners < RARE_MAX_LISTENERS:
                    ...
                    pass
        pool = rare_pool if len(rare_pool) >= 10 else pool
    elif mode == "dig":
        pool = _expand_era_pool(decades, depth=1)
    else:  # playlist
        pool = _expand_era_pool(decades, depth=0)

    # Confirm era with MusicBrainz filter
    if message:
        _safe_reply(message, "🔍 Verifying artists…")
    filtered = _filter_artists_by_decade(pool, decades, message=None)
...
def handle_buttons(update, context):
    ...
    # ── decade_confirm: execute pending generation ────────────────────────────
        ...
        if gen_action == "playlist":
            ...
            result      = select_tracks_with_decades(expand_artist_graph(seeds), decades=decades, message=message, mode="playlist")
            _cancel_working(sent, timer)
            ...
            result      = select_tracks_with_decades(expand_artist_graph_deep(seeds), decades=decades, message=message, mode="dig")
            _cancel_working(sent, timer)
            era_tag = f" — {_decade_label_from_set(decades)}" if decades else ""
            ...
            result      = select_tracks_with_decades(expand_artist_graph_rare(extract_seed_artists()), decades=decades, message=message, mode="rare")
            _cancel_working(sent, timer)
            era_tag = f" — {_decade_label_from_set(decades)}" if decades else ""
    ...
    # ── explore_artist: navigate to a similar artist's map card ──────────────
        ...
        # Edit current message to "Mapping..." — will be deleted after card loads
        try: query.message.edit_text(f"🔭 Mapping {new_artist.upper()}…")
        except Exception: pass
        _render_map(message, new_artist, chat_id)
        # Delete the "Mapping..." message now that card is shown
