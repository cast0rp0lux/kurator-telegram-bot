...
# ─── Version ──────────────────────────────────────────────────────────────────
BOT_VERSION = "Kurator 📀 Music Discovery Engine (v4.8.11)"
...
def _fetch_track_from_early_albums(artist, decade_years=None):
    ...
    try:
        ...
        # Step 2: verify artist was active in the requested decade
        if decade_years:
            active_decades = _artist_decade_from_mb(artist)
            if active_decades:
                lo, hi = decade_years
                artist_in_era = any(
                    DECADE_YEARS[d][0] <= hi and DECADE_YEARS[d][1] >= lo
                    for d in active_decades
                )
                if not artist_in_era:
                    log.info(f"Era mismatch: '{artist}' active in {active_decades}, not in {decade_years}")
                    return None  # discard — wrong era

        # Step 3: get studio albums ordered by year
        ...
        if decade_years:
            ...
            candidate_albums = era_albums[:5] if era_albums else albums[:4]
        else:
...
def send_playlist(message, tracks, title="✦ Kurator's Playlist", branded=True, chat_id=None, size=None, map_chat_id=None, suppress_warning=False):
    ...
    if not suppress_warning and len(tracks) < target * 0.7:
        short_warning = f"\nOnly {len(tracks)} tracks found — history is getting full (/clear to refresh).\n"
...
/clear — Clear track history and start fresh
...
def _show_clear_confirm(query):
    query.edit_message_text(
        "🗑️ Clear history?\n\n"
        "Your track history will be deleted.\n"
        "Tags and map sessions are kept.",
        reply_markup=InlineKeyboardMarkup([
...
def handle_buttons(update, context):
    ...
    # ── decade_open: show decade toggle grid ─────────────────────────────────
        ...
        # Extract genre from pending_gen if available
    ...
    # ── decade_confirm: execute pending generation ────────────────────────────
        ...
        if gen_action == "playlist":
            ...
            query.edit_message_text(f"🎸 Building {style.title()} playlist…")
            ...
            result = select_tracks_with_decades([], size=GENRE_PLAYLIST_SIZE,
                                                ...
                                                genre=style)
            _cancel_working(sent, timer)
            era_tag  = f" — {_decade_label_from_set(decades)}"
            from_map = chat_id in map_memory
            send_playlist(message, result, title=f"🎸 {style.title()}{era_tag}",
    ...
    # ── tags ──────────────────────────────────────────────────────────────────
        ...
        if value in tag_index:
            del tag_index[value]
            save_tag_index()
        # Add to personal blacklist so it never comes back
        tag_blacklist.add(value.lower())
        save_tag_blacklist()
        all_tags    = sorted(tag_index.items(), key=lambda x: x[1], reverse=True)
        sorted_tags = [(t, c) for t, c in all_tags if _is_valid_tag(t)]
        if not sorted_tags:
        ...
        query.edit_message_text(
            ...
            reply_markup=InlineKeyboardMarkup([
                ...
                [InlineKeyboardButton("← Cancel",        callback_data="cmd|tags")],
            ])
        )
