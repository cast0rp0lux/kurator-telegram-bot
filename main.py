...
# ─── Version ──────────────────────────────────────────────────────────────────
BOT_VERSION = "Kurator 📀 Music Discovery Engine (v3.7.0)"
...
def spotify_check_premium(token):
    ...
    try:
        ...
        return product == "premium", user_id
    except Exception as e:
...
def spotify_build_playlist(chat_id, tracks, title):
    """
    Full flow: check Premium, search URIs in parallel, create playlist, add tracks.
    Returns (playlist_url, error_code) where error_code is None on success,
    'not_premium' if user lacks Premium, or 'failed' on other errors.
    """
    token = get_spotify_token(chat_id)
    if not token:
        return None, "no_token"

    is_premium, user_id = spotify_check_premium(token)
    if not user_id:
        return None, "failed"
    if not is_premium:
        log.warning(f"Spotify: user {user_id} is not Premium.")
        return None, "not_premium"

    # Search URIs in parallel
    uris = []
...
def qobuz_url(track):
    # Correct Qobuz search URL format
    return f"https://play.qobuz.com/search#query={quote(track)}&type=tracks"
...
def _get_artist_info(artist_query):
    ...
    try:
        r = requests.get(
            ...
            params={"query": f'artist:"{artist_query}"', "fmt": "json", "limit": 1},
            headers={"User-Agent": "Kurator/3.7.0 (telegram bot)"},
            timeout=10
        )
        if r.status_code == 200:
            ...
            if artists:
                mb = artists[0]
                code = mb.get("country") or (mb.get("area") or {}).get("iso-3166-1-codes", [None])[0]
                if code:
...
def send_playlist(message, tracks, title="✦ Kurator's Pick", branded=True, chat_id=None, size=None):
    ...
    if chat_id and get_spotify_token(chat_id):
        ...
        elif err == "failed" or err == "no_uris":
