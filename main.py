...
# ─── Version ──────────────────────────────────────────────────────────────────
BOT_VERSION = "Kurator 📀 Music Discovery Engine (v4.6.4)"
...
# Curated styles per decade
DECADE_STYLES = {
    "50s": ["Rock & Roll", "Rhythm & Blues", "Rockabilly", "Doo Wop", "Jump Blues"],
    "60s": ["Psychedelic Rock", "Garage Rock", "Beat", "Freakbeat", "Mod", "Surf",
            "Folk Rock", "British Blues", "Soul", "Bossa Nova"],
    "70s": ["Krautrock", "Prog Rock", "Canterbury Scene", "Jazz-Rock", "Funk",
    ...
    "00s": ["Post-Punk Revival", "Noise Pop", "Freak Folk", "Chamber Pop",
            "Indie Folk", "Math Rock", "Neo-Psychedelia"],
    "10s": ["Chillwave", "Witch House", "Ambient Pop", "Indie Folk", "Post-Rock"],
    "20s": ["Lo-Fi", "Bedroom Pop", "Ambient", "Indie Pop", "Neo-Soul"],
}
...
def _get_era_artists_from_discogs(decades, mode="playlist", max_artists=80):
    ...
    for style in styles[:8]:
        if len(artists) >= max_artists:
            break
        try:
            years = random.sample(range(year_lo, year_hi + 1),
                                  min(5, year_hi - year_lo + 1))
            for year in years:
                r = requests.get(
    ...
    log.info(f"Discogs era pool: {len(artists)} artists for {decades} mode={mode}")
    return list(artists)
...
def select_tracks_with_decades(artists, size=None, decades=None, message=None, mode="playlist"):
    ...
    mode controls mainstream level: playlist > dig > rare
    """
    if not decades:
    ...
    if mode == "dig":
        # Discogs pool → expand with Last.fm similars
        ...
        if len(pool) < 15:
            pool = seeds  # fallback to seeds if expansion too small
    elif mode == "rare":
    ...
    return select_tracks(pool, size=size)
