...
def _nav(query, text, markup=None, disable_preview=False):
    ...
    This forces Telegram to scroll to the bottom automatically.
    Falls back to edit_message_text if delete fails.
    """
    try:
        ...
        if markup:
            ...
            query.message.reply_text(text, disable_web_page_preview=disable_preview)
    except Exception:
        try:
            if markup:
                query.message.reply_text(text, reply_markup=markup,
                ...
                _nav(query, text, disable_web_page_preview=disable_preview)
