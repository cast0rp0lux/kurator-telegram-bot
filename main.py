...
def select_tracks_with_decades(artists, size=None, decades=None, message=None, mode="playlist", style_filter=None, use_mb=False):
    ...
    Cross-validates artists against Last.fm tags for the same genre — filters noise.
    use_mb: additionally validate artists via MusicBrainz begin_year.
    """
    if not decades:
    ...
    if mode == "dig":
        seeds = _get_era_artists_from_discogs(decades, mode="dig", max_artists=80)
        if message:
        ...
        pool = _get_era_artists_from_discogs(decades, mode="rare", max_artists=120)
        ...
        with ThreadPoolExecutor(max_workers=10) as ex:
            ...
            for f in as_completed(futures):
                try:
                    ...
                    pass
        pool = rare_pool if rare_pool else pool[:20]  # never revert to unfiltered
        log.info(f"Rare era: {len(pool)} after listeners filter")
    else:
        pool = _get_era_artists_from_discogs(decades, mode="playlist", max_artists=100)
