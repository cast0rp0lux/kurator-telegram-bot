...
def handle_buttons(update, context):
    ...
    # ── decade_confirm: execute pending generation ────────────────────────────
        ...
        if gen_action == "playlist":
            ...
            # Lite: tag.gettoptracks → tracks directly, single API call
            tracks = _get_genre_tracks_lite(style, decades, size=GENRE_PLAYLIST_SIZE)

            # Fallback to Discogs + top_track if tag returns nothing
            if not tracks:
                log.info(f"tag.gettoptracks empty for '{style}' — falling back to Discogs")
                ...
                if pool:
                    ...
                    artists_used = []
                    with ThreadPoolExecutor(max_workers=8) as ex:
                        futures = [ex.submit(_fetch_top_track, a) for a in pool[:GENRE_PLAYLIST_SIZE * 3]]
                        for f in as_completed(futures):
