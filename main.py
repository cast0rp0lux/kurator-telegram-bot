...
def select_tracks_with_decades(artists, size=None, decades=None, message=None, mode="playlist", style_filter=None, use_mb=False, genre=None):
    ...
    if mode == "dig":
        ...
        if len(pool) < 15:
            pool = seeds
        # Cap before verification — 300 is more than enough for 50 tracks
        if len(pool) > 300:
            random.shuffle(pool)
            pool = pool[:300]
    elif mode == "rare":
    ...
    if use_mb and verified and decades:
        ...
        mb_passed  = []
        mb_unknown = []
        target_mb  = (size or PLAYLIST_SIZE) * 3  # stop early — enough for track selection
        for artist in verified:
            if len(mb_passed) >= target_mb:
                break
            try:
                active = _artist_decade_from_mb(artist)
                if not active:
