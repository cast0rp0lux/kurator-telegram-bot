...
# ─── Version ──────────────────────────────────────────────────────────────────
BOT_VERSION = "Kurator 📀 Music Discovery Engine (v4.9.0)"
...
ERA_HAVE_RANGES = {
    "playlist": (200, 15000),  # known, well documented
    "dig":      (30,  3000),   # less obvious
...
def _get_era_tracks_from_discogs(decades, style=None, mode="playlist", max_tracks=200):
    ...
    # Step 1: Search releases in parallel
    tasks = []
    for s in styles[:10]:
        years = random.sample(all_years, min(6, len(all_years)))
        for year in years:
            tasks.append((s, year))
        ...
        try:
            ...
            for rel in r.get("results", []):
                ...
                if have_min <= have <= have_max:
                    results.append(rel.get("id"))
            return results
        except Exception as e:
            log.error(f"Discogs search: {e}")
            return []
    ...
    # Step 2: Get tracklists from releases in parallel
    ...
    raw_pairs = []
    with ThreadPoolExecutor(max_workers=10) as ex:
        futures = [ex.submit(_get_tracklist, rid) for rid in release_ids[:80]]
        for f in as_completed(futures):
            raw_pairs.extend(f.result())
            if len(raw_pairs) >= max_tracks:
                break

    log.info(f"Era tracks from Discogs: {len(release_ids)} releases → {len(raw_pairs)} tracks ({mode})")
    return raw_pairs  # list of (artist, track_title) tuples
    """
    ...
    Calls are parallelized for speed.
    """
    ...
    # Build all (style, year) pairs upfront
    tasks = []
...
def select_tracks_with_decades(artists, size=None, decades=None, message=None, mode="playlist", style_filter=None, use_mb=False):
    ...
    # For Rare: filter out mainstream artists by Last.fm listeners
    if mode == "rare":
        if message:
    ...
    log.info(f"Era playlist ({mode}): {len(result)} tracks selected")
    return result
