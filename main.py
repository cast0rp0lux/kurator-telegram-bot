...
# ─── Version ──────────────────────────────────────────────────────────────────
BOT_VERSION = "Kurator 📀 Music Discovery Engine (v4.9.2)"
...
def _clean_track_title(title):
    """Aggressive cleaning — keep only the actual song title, nothing else."""
    import re

    noise_patterns = [
        # Translations: "Title = Traducción"
        r'\s*=\s*.+$',
        # Track position prefix: "03 - ", "A1 - ", "B2. ", "03. "
        r'^\s*[A-Za-z]?\d{1,2}\s*[\.\-\u2013]\s*',
        # Remaster/reissue variants
        ...
        # Part indicators: (Part 1), (Pt. II), Part 2, etc.
        r'\s*[\(\[]\s*(?:part|pt\.?)\s*[ivxIVX\d]+\s*[\)\]]',
        r'\s*[-–]\s*(?:part|pt\.?)\s*[ivxIVX\d]+\s*$',
        # Standalone year: (1973), [1982]
        r'\s*[\(\[]\s*(?:19|20)\d{2}\s*[\)\]]',
        # Numbered duplicates: [2], [3], (2)
        r'\s*[\(\[]\s*\d+\s*[\)\]]',
        # File-style prefix: "06-artist-name-"
        r'^\d{2}-[^-]+-',
    ]
...
def select_tracks(artists, size=None, skip_recent=True):
    ...
    keys_added = set()
    artists_used = []
    # Filter blacklisted and optionally recently used artists
...
ERA_HAVE_RANGES = {
    "playlist": (50,  15000),  # known, findable in streaming
    "dig":      (20,  5000),   # less obvious
    "rare":     (5,   500),    # very obscure releases
}

# Map generic user tags to specific Discogs styles
TAG_TO_DISCOGS_STYLES = {
    "rock":          ["Hard Rock", "Classic Rock", "Album Rock", "Prog Rock", "Psychedelic Rock"],
    "electronic":    ["Synth-Pop", "Ambient", "Industrial", "EBM", "Minimal Wave"],
    "jazz":          ["Jazz-Rock", "Free Jazz", "Fusion", "Hard Bop", "Cool Jazz"],
    "folk":          ["Folk Rock", "Folk", "Acoustic", "Singer/Songwriter"],
    "metal":         ["Heavy Metal", "Thrash Metal", "Doom Metal", "Black Metal"],
    "punk":          ["Punk", "Hardcore", "Post-Punk", "Proto-Punk"],
    "blues":         ["Electric Blues", "Chicago Blues", "British Blues", "Blues Rock"],
    "soul":          ["Soul", "Funk", "Rhythm & Blues", "Northern Soul"],
    "pop":           ["Pop Rock", "Synth-Pop", "Indie Pop", "Chamber Pop"],
    "country":       ["Country Rock", "Country", "Americana", "Alt-Country"],
    "reggae":        ["Reggae", "Dub", "Ska", "Rocksteady"],
    "hip hop":       ["Hip Hop", "Boom Bap", "Conscious", "Gangsta"],
    "hip-hop":       ["Hip Hop", "Boom Bap", "Conscious"],
    "classical":     ["Contemporary", "Modern Classical", "Minimalism", "Orchestral"],
    "ambient":       ["Ambient", "Dark Ambient", "Drone", "New Age"],
    "experimental":  ["Experimental", "Noise", "Avant-garde", "Free Improvisation"],
    "dance":         ["House", "Techno", "Disco", "Electronic"],
}

def _resolve_discogs_styles(tag):
    """Map a user tag to Discogs styles. Returns list of styles to try."""
    tag_lower = tag.lower().strip()
    if tag_lower in TAG_TO_DISCOGS_STYLES:
        return TAG_TO_DISCOGS_STYLES[tag_lower]
    return [tag]  # use as-is if no mapping

def _get_era_tracks_from_discogs(decades, style=None, mode="playlist", max_tracks=200):
    """
    New era algorithm: fetch releases directly from Discogs, extract tracklists.
    Year of each track is guaranteed by the release year.
    """
    have_min, have_max = ERA_HAVE_RANGES.get(mode, (50, 15000))

    # Build style list — resolve generic tags to specific Discogs styles
    if style:
        styles = _resolve_discogs_styles(style)
        years_per_style = max(4, 10 // len(styles))
    else:
        styles = []
        for d in decades:
    ...
    # Step 1: Search releases — rate limited to avoid Discogs 429
    tasks = []
    for s in styles[:10]:
        years = random.sample(all_years, min(years_per_style, len(all_years)))
        for year in years:
            tasks.append((s, year))

    import time as _time

    def _search_one(s, year):
        try:
            r = requests.get(
            ...
            for rel in data.get("results", []):
                ...
                if have_min <= have <= have_max:
                    rel_id = rel.get("id")
                    if rel_id:
                        results.append(rel_id)
            return results
        except Exception as e:
            log.error(f"Discogs search {s}/{year}: {e}")
            return []

    # Use fewer workers to stay within Discogs rate limit
    release_ids = []
    with ThreadPoolExecutor(max_workers=6) as ex:
        futures = [ex.submit(_search_one, s, y) for s, y in tasks]
        for f in as_completed(futures):
            release_ids.extend(f.result())

    random.shuffle(release_ids)
    release_ids = list(dict.fromkeys(release_ids))  # deduplicate
    log.info(f"Era search: {len(tasks)} queries → {len(release_ids)} releases ({mode})")

    if not release_ids:
        return []

    # Step 2: Get tracklists from releases — rate limited
    def _get_tracklist(release_id):
        try:
            r = requests.get(
            ...
            # Get artist name, clean Discogs disambiguation "(2)"
            artists = data.get("artists", [])
            if not artists:
                return []
            artist_name = artists[0].get("name", "").strip()
            artist_name = re.sub(r'\s*\(\d+\)$', '', artist_name).strip()
            if not artist_name or _is_blacklisted(artist_name):
                return []

            result = []
            for track in data.get("tracklist", []):
                t_type  = track.get("type_", "track")
                t_title = track.get("title", "").strip()
                if t_type != "track" or not t_title:
                    continue
                if _is_live_track(t_title):
                    continue
                t_title = _clean_track_title(t_title)
                if not t_title or len(t_title) < 2:
                    continue
                result.append((artist_name, t_title))
            return result
        except Exception as e:
            log.error(f"Discogs tracklist {release_id}: {e}")
            return []

    raw_pairs = []
    with ThreadPoolExecutor(max_workers=6) as ex:
        futures = [ex.submit(_get_tracklist, rid) for rid in release_ids[:60]]
        for f in as_completed(futures):
            raw_pairs.extend(f.result())
            if len(raw_pairs) >= max_tracks:
                break

    log.info(f"Era tracklists: {len(release_ids)} releases → {len(raw_pairs)} tracks ({mode})")
    return raw_pairs
...
def _get_style_tracks_from_discogs(style, mode="playlist", max_tracks=300):
    """
    Free Explore without era — same Discogs approach but no year filter.
    Returns (artist, track_title) pairs.
    """
    have_min, have_max = ERA_HAVE_RANGES.get(mode, (50, 15000))
    styles = _resolve_discogs_styles(style)
    import time as _time

    def _search_one(s, page):
        try:
            r = requests.get(
                "https://api.discogs.com/database/search",
                params={
            ...
            log.error(f"Discogs style search {s}: {e}")
            return []

    # Search across styles and pages for variety
    tasks = []
    pages_per_style = max(2, 6 // len(styles))
    for s in styles:
        for p in random.sample(range(1, 10), min(pages_per_style, 9)):
            tasks.append((s, p))

    release_ids = []
    with ThreadPoolExecutor(max_workers=6) as ex:
        futures = [ex.submit(_search_one, s, p) for s, p in tasks]
        for f in as_completed(futures):
            release_ids.extend(f.result())

    random.shuffle(release_ids)
    release_ids = list(dict.fromkeys(release_ids))
    log.info(f"Style search '{style}': {len(release_ids)} releases ({mode})")
    ...
    log.info(f"Style tracks '{style}': {len(raw_pairs)} tracks ({mode})")
    return raw_pairs

def _select_from_pairs(raw_pairs, size, message=None):
    """Select tracks from (artist, title) pairs, filtering history.
    Rules: max 1 track per artist, no recently used artists.
    """
    global _recent_artists
    if message:
    ...
    seen_keys    = set()
    seen_artists = set()  # max 1 per artist per playlist
    result       = []
    keys_added   = []
    artists_used = []
    for artist, track_title in raw_pairs:
        if len(result) >= size:
            break
        # Skip if artist already in this playlist
        artist_norm = normalize(artist)
        if artist_norm in seen_artists:
            continue
        # Skip if artist used recently
        if artist_norm in recent_set:
            continue
        key = f"{artist_norm}-{normalize(track_title)}"
        if key in seen_keys or track_in_history(key):
            continue
        seen_keys.add(key)
        seen_artists.add(artist_norm)
        result.append(f"{artist} - {track_title}")
...
def select_tracks_with_decades(artists, size=None, decades=None, message=None, mode="playlist", style_filter=None, use_mb=False):
    """Free Explore era playlists — tracks sourced directly from Discogs releases."""
    if not decades:
        return select_tracks(artists, size=size)

    target = size or PLAYLIST_SIZE
    style  = style_filter[0] if style_filter else None

    if message:
        _safe_reply(message, "🔍 Building era pool…")

    raw_pairs = _get_era_tracks_from_discogs(
        decades, style=style, mode=mode, max_tracks=target * 6
    )

    if not raw_pairs:
        log.info("Era tracks empty — falling back to user history pool")
        return select_tracks(artists, size=size)

    if mode == "rare":
        if message:
            _safe_reply(message, "💎 Filtering for obscure artists…")
        artist_names = list({a for a, _ in raw_pairs})
        listener_map = {}
        with ThreadPoolExecutor(max_workers=10) as ex:
            futures = {ex.submit(_fetch_listeners, a): a for a in artist_names}
            for f in as_completed(futures):
                try:
                    a, listeners = f.result()
                    listener_map[a] = listeners
                except Exception:
                    pass
        raw_pairs = [(a, t) for a, t in raw_pairs
                     if 0 < listener_map.get(a, 1) < RARE_MAX_LISTENERS]
        log.info(f"Rare: {len(raw_pairs)} tracks after listener filter")

    return _select_from_pairs(raw_pairs, target, message=message)
...
def _decade_selector_buttons(chat_id):
    ...
    if row:
        buttons.append(row)
    label = f"🍌 GENERATE PLAYLIST — {_decade_label(chat_id)}" if selected else "🍌 GENERATE PLAYLIST (any era)"
...
def _show_era_choice(query, chat_id, title, gen_action, back_cb):
    ...
    query.edit_message_text(
        f"{title} — Select era\n\n"
        f"📡 Decade filtering takes a little longer.\n"
        f"Kurator cross-checks each artist against multiple sources.\n"
        f"Usually under 60 seconds. Be patient.",
        reply_markup=InlineKeyboardMarkup([
            [InlineKeyboardButton("♾️ Any era",    callback_data="decade_confirm")],
...
def main_menu_markup():
    return InlineKeyboardMarkup([
        ...
        [InlineKeyboardButton("🎵🏷️ /playlist <tag> — Genre playlists", callback_data="cmd|genre_prompt")],
...
def playlist(update, context):
    ...
    if context.args:
        ...
        # Store tag and show era selector
        _pending_gen[chat_id] = {"action": f"build|{tag}", "back": "cmd|menu"}
        _pending_decades.pop(chat_id, None)
        msg.reply_text(
            f"🎵🏷️ {tag.title()} playlist\n\nSelect era:",
            reply_markup=InlineKeyboardMarkup([
                [InlineKeyboardButton("♾️ Any era",    callback_data="decade_confirm")],
                [InlineKeyboardButton("📅 Select decade", callback_data="decade_open")],
                [InlineKeyboardButton("📀 Main menu",     callback_data="cmd|menu")],
...
def handle_buttons(update, context):
    ...
    # ── decade_confirm: execute pending generation ────────────────────────────
        ...
        if gen_action == "playlist":
            ...
            sent, timer = _working_message(message, "Going deeper…", delay=50)
            if decades:
            ...
            query.edit_message_text(f"🎵🏷️ Building {style.title()} playlist…")
            sent, timer = _working_message(message, "Still building…", delay=50)
            if decades:
                result = select_tracks_with_decades([], size=GENRE_PLAYLIST_SIZE,
                                                    decades=decades, message=message,
                                                    mode="playlist", style_filter=[style])
            else:
                # Free Explore without era — Discogs, no year filter
                if message:
                    _safe_reply(message, "🔍 Building era pool…")
                raw_pairs = _get_style_tracks_from_discogs(style, mode="playlist",
                                                           max_tracks=GENRE_PLAYLIST_SIZE * 6)
                result = _select_from_pairs(raw_pairs, GENRE_PLAYLIST_SIZE, message=message)
                if not result:
                    # Fallback to Last.fm if Discogs returns nothing
                    ...
                    result = select_tracks(names, size=GENRE_PLAYLIST_SIZE, skip_recent=False)
            _cancel_working(sent, timer)
            era_tag  = f" — {_decade_label_from_set(decades)}" if decades else ""
            from_map = chat_id in map_memory
            send_playlist(message, result, title=f"🎵🏷️ {style.title()}{era_tag}",
    ...
    # ── onboarding ────────────────────────────────────────────────────────────
        ...
        if step == 1:
            ...
            query.edit_message_text(
                ...
                "🍌 KURATOR'S PICKS\n"
                "Digital digging from a real listening history. Curated, not generated.\n\n"
                "🔍 FREE EXPLORE\n"
    ...
    # ── build: playlist from style — era choice first ─────────────────────────
        ...
        style = value
        _pending_decades.pop(chat_id, None)
        _show_era_choice(query, chat_id, f"🎵🏷️ {style.title()}", f"build|{style}", f"map_back|{chat_id}")
