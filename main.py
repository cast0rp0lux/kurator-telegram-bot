...
# ─── Version ──────────────────────────────────────────────────────────────────
BOT_VERSION = "Kurator 📀 Music Discovery Engine (v3.5.0)"
...
PLAYLIST_SIZE       = 30        # default playlist size
GENRE_PLAYLIST_SIZE = 50        # larger size for genre/tag playlists
...
CORE_RELEASES       = 5         # top Discogs releases to mine for /core
...
# ─── /core: genre fundamentals via Discogs ────────────────────────────────────

def _get_core_artists_for_genre(genre):
    """
    Fetch top Discogs releases for a genre/style, extract artists.
    These are editorial picks (most collected/rated), not streaming popularity.
    """
    try:
        data = requests.get(
            ...
            params={
                ...
                "type":  "release",
                "per_page": 50,
                "sort": "want",       # sorted by most wanted = most valued
                "token": DISCOGS_TOKEN,
            },
            timeout=15
        ).json()
    except Exception as e:
        log.error(f"Discogs core error: {e}")
        return []
    ...
    return artists[:40]  # cap at 40 artists
...
def main_menu_markup():
    return InlineKeyboardMarkup([
        ...
        [InlineKeyboardButton("🎯 Core — Genre fundamentals",         callback_data="cmd|core_prompt")],
        [
...
🎯 /core <genre> — Genre fundamentals, not the obvious hits
...
def trail(update, context):
    ...
    msg.reply_text("🧬 Following the trail…")
...
def core(update, context):
    """Genre fundamentals — editorial picks from Discogs, not streaming popularity."""
    msg     = update.message
    chat_id = update.effective_chat.id
    if not context.args:
        msg.reply_text("🎯 Core\n\nSend: /core <genre>\nExample: /core krautrock")
        return
    genre = " ".join(context.args)
    msg.reply_text(f"🎯 Mining the fundamentals of {genre}…")
    artists = _get_core_artists_for_genre(genre)
    if not artists:
        msg.reply_text(f"No fundamental releases found for \"{genre}\".\nTry a different genre or spelling.")
        return
    msg.reply_text(f"Found {len(artists)} key artists. Selecting tracks…")
    send_playlist(msg, select_tracks(artists, size=GENRE_PLAYLIST_SIZE),
                  title=f"🎯 {genre} — Core", branded=False, chat_id=chat_id, size=GENRE_PLAYLIST_SIZE)
...
def handle_buttons(update, context):
    ...
    if action == "cmd":
        ...
        elif value == "core_prompt":
            query.edit_message_text("🎯 Core\n\nSend:\n/core <genre>\nExample: /core krautrock")
...
dp.add_handler(CommandHandler("core",        core))
