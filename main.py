...
def _artist_matches_genre(artist, valid_genres):
    """Return True if artist has at least one tag overlapping with valid_genres."""
    try:
        data = lastfm("artist.getinfo", artist=artist)
        ...
        return bool(artist_tags & valid_genres)
    except Exception:
        return True  # if we can't check, give benefit of the doubt
...
def _get_era_artists_from_lastfm(genre, decades, max_artists=150):
    ...
    seed_pool = top_names[20:150] if len(top_names) > 20 else top_names
