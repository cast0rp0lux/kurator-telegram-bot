...
# ─── Version ──────────────────────────────────────────────────────────────────
BOT_VERSION = "Kurator 📀 Music Discovery Engine (v3.9.0)"
...
TRACK_LINKS_PAGE    = 10
MB_USER_AGENT       = "Kurator/3.9.0 (telegram bot)"
...
def _mb_artist_full(mbid):
    """Full artist lookup with tags."""
    return _mb_get(f"artist/{mbid}", {"inc": "tags"})

def _mb_studio_albums(mbid):
    """
    Fetch studio albums (release-groups of type Album) ordered by year asc.
    Returns list of {title, year}.
    """
    ...
    for rg in data.get("release-groups", []):
        if rg.get("primary-type") == "Album":
            ...
            albums.append({"title": rg.get("title", ""), "year": year})
    albums.sort(key=lambda a: a["year"] or "9999")
    return albums

def _mb_label_artists(label_name):
    """
    Search MusicBrainz for artists associated with a label.
    Returns list of artist names.
    """
    data = _mb_get("artist/", {"query": f'label:"{label_name}"', "limit": 100})
    return [a.get("name") for a in data.get("artists", []) if a.get("name")]
...
🗺️ /map <artist> — Explore an artist's world
...
def start(update, context):
    ...
    if is_onboarded(chat_id):
        ...
        update.message.reply_text(
            ...
            reply_markup=InlineKeyboardMarkup([[
                InlineKeyboardButton("→ What is Kurator?", callback_data="onboard|2")
            ]])
        )
...
def _build_map_buttons(display_name, sorted_styles, info, chat_id):
    ...
    if label:
        label_short = label[:18]
        buttons.append([
            InlineKeyboardButton(f"🏷️ {label_short} — Hidden", callback_data=safe_callback(f"label_play|rare|{label}")),
            InlineKeyboardButton("All", callback_data=safe_callback(f"label_play|all|{label}")),
        ])

    # Genre pills — 2 per row, clicables
    if info.get("genres"):
        row = []
        for g in info["genres"]:
            row.append(InlineKeyboardButton(g, callback_data=safe_callback(f"map_style|{g}")))
            if len(row) == 2:
                buttons.append(row)
                row = []
        if row:
...
def handle_buttons(update, context):
    ...
    # ── onboarding ────────────────────────────────────────────────────────────
        ...
        if step == 2:
            query.edit_message_text(
                ...
                reply_markup=InlineKeyboardMarkup([[
                    InlineKeyboardButton("→ How does it work?", callback_data="onboard|3")
                ]])
            )
        elif step == 3:
            mark_onboarded(chat_id)
            query.edit_message_text(
                "Kurator connects to Last.fm, Discogs and MusicBrainz to build playlists "
                "from real data — no AI, no guessing.\n\n"
                "Export via Soundiiz to Spotify, Qobuz, Apple Music or any platform.",
                reply_markup=InlineKeyboardMarkup([[
    ...
    # ── map_albums ────────────────────────────────────────────────────────────
        ...
        buttons = []
        for album in albums:
            label = f"{album['title'][:32]}  ·  {album['year']}" if album.get("year") else album["title"][:40]
            buttons.append([InlineKeyboardButton(
                label,
    ...
    # ── album_select ──────────────────────────────────────────────────────────
        ...
        for a in info.get("albums", []):
            if a["title"].startswith(album_title):
                ...
                break
        query.edit_message_text(
            f"{display_name} — {full_title.upper()}",
            reply_markup=InlineKeyboardMarkup([
    ...
    # ── album_playlist ────────────────────────────────────────────────────────
        ...
        if not track_strings:
            ...
            return
        send_playlist(message, track_strings, title=f"💿 {album.upper()}", branded=False, chat_id=chat_id)
    ...
    # ── map_similar ───────────────────────────────────────────────────────────
        ...
        query.edit_message_text(
            ...
            reply_markup=InlineKeyboardMarkup([
                ...
                [InlineKeyboardButton(f"⬅ Back", callback_data=f"map_back|{chat_id}")],
            ])
        )
    ...
    # ── trail_go ──────────────────────────────────────────────────────────────
        ...
        query.edit_message_text(f"🧬 {artist} — Following the trail ({labels.get(hops, '')})…")
