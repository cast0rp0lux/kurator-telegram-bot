...
# ─── Version ──────────────────────────────────────────────────────────────────
BOT_VERSION = "Kurator 📀 Music Discovery Engine (v4.8.1)"
...
def select_tracks_with_decades(artists, size=None, decades=None, message=None, mode="playlist", style_filter=None, use_mb=False):
    ...
    if mode == "dig":
        ...
        pool = _get_era_artists_from_discogs(decades, mode="rare", max_artists=100)
    else:
    ...
    # Cross-validate: get Last.fm tag artists for the same styles
    # Only keep artists that appear in BOTH Discogs and Last.fm for the genre
    if style_filter:
        if message:
            _safe_reply(message, "🔍 Cross-checking sources…")
        lfm_artists = set()
        for tag in style_filter[:3]:  # check up to 3 tags
            try:
                data  = lastfm("tag.gettopartists", tag=tag, limit=250)
                items = data.get("topartists", {}).get("artist", [])
                for a in items:
                    name = a.get("name", "")
                    if name:
                        lfm_artists.add(normalize(name))
            except Exception:
                pass
        if lfm_artists:
            cross = [a for a in pool if normalize(a) in lfm_artists]
            log.info(f"Cross-validation: {len(pool)} Discogs → {len(cross)} in Last.fm too")
            pool = cross if len(cross) >= 10 else pool
