...
def _export_buttons(key, spotify_url_playlist=None, chat_id=None):
    """Reusable export button set — used by send_playlist and back navigation."""
    buttons = [
        [InlineKeyboardButton("📤 Export via Soundiiz",
                              url="https://soundiiz.com")],
    ]
    if spotify_url_playlist:
        buttons.append([InlineKeyboardButton("🎧 Export to Spotify",
                                             url=spotify_url_playlist)])
    else:
        buttons.append([InlineKeyboardButton("🎧 Export to Spotify",
                                             callback_data=f"sp_build|{key}")])
    buttons.append([InlineKeyboardButton("🔗 Open track links",
                                         callback_data=f"sp_expand|{key}")])
...
def send_playlist(message, tracks, title="✦ Kurator's Pick", branded=True, chat_id=None):
    ...
    # Playlist message — header + version + tracks
    message.reply_text(
    ...
    message.reply_text(
        ...
        disable_web_page_preview=True,
        reply_markup=InlineKeyboardMarkup(
            _export_buttons(key, spotify_url_playlist, chat_id)
        )
    )
...
def handle_buttons(update, context):
    ...
    # ── sp_build: create Spotify playlist on demand (if not done at generation) ─
        ...
        if url:
            ...
            query.edit_message_text(
                "❌ Could not create playlist. Check logs.",
                reply_markup=InlineKeyboardMarkup([[
                    InlineKeyboardButton("⬅ Back", callback_data=f"export_back|{key}")
                ]])
            )
