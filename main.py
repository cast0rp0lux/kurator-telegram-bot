...
def score_artist_tags(tags, target_genre):
    score = 0
    if target_genre in tags:
        score += 3
    rel = GENRE_GRAPH.get(target_genre, {})
    if any(t in rel.get("near", []) for t in tags):
        score += 2
    if any(t in rel.get("mid", []) for t in tags):
        score += 1
    if any(t in ["pop","edm","trap","reggaeton"] for t in tags):
        score -= 2
    return score

def filter_artist_pool_by_tags(artists, genre):
    scored = []
    with ThreadPoolExecutor(max_workers=6) as ex:
        futures = {ex.submit(get_artist_tags, a): a for a in artists}
        for f in as_completed(futures):
            a = futures[f]
            try:
                tags = f.result()
                s = score_artist_tags(tags, genre)
                if s > 0:
                    scored.append((a, s))
            except:
                continue
...
def select_tracks_v5(artists, genre=None, size=50):
    artists = filter_artist_pool_by_tags(artists, genre) if genre else artists

    target = size
    tracks = []
    keys_added = set()

    sparse = len(artists) < 40
    max_fallback = int(target * (0.2 if sparse else 0.35))
    fallback_count = 0

    random.shuffle(artists)

    with ThreadPoolExecutor(max_workers=4) as ex:
        futures = [ex.submit(_fetch_track_from_early_albums, a) for a in artists[:target*4]]
        ...
        for f in as_completed(futures):
            ...
            try:
                ...
                if result:
                    artist, track_name, key = result
                    if key not in keys_added and not track_in_history(key):
                        tracks.append(f"{artist} - {track_name}")
                        keys_added.add(key)
                else:
                    if fallback_count < max_fallback:
                        fb = _fetch_top_track(artist)
                        if fb:
                            artist, track_name, key = fb
                            tracks.append(f"{artist} - {track_name}")
                            keys_added.add(key)
                            fallback_count += 1
            except:
