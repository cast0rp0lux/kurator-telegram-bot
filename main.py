...
# ─── Version ──────────────────────────────────────────────────────────────────
BOT_VERSION = "Kurator 📀 Music Discovery Engine (v4.6.0)"
...
# ─── MusicBrainz decade filter — cache ───────────────────────────────────────
...
Kurator cross-checks each artist against
multiple sources (usually under 60 seconds).
...
def _render_map(message, artist_query, chat_id):
    # Fetch artist info first to get official name for Discogs
...
def handle_buttons(update, context):
    ...
    # ── map_similar ───────────────────────────────────────────────────────────
        ...
        # Sort by name length so shorter names pair better in 2 columns
        preview = sorted(similar[:12], key=lambda x: len(x))
        rest    = similar[12:]
        ...
        # 2 columns — pair by length (shortest with shortest)
        i = 0
        while i < len(preview):
            name = preview[i]
            if i + 1 < len(preview):
                name2 = preview[i+1]
                # If both short enough, put in same row
                if len(name) + len(name2) <= 28:
                    buttons.append([
                        InlineKeyboardButton(name.upper(),  callback_data=safe_callback(f"explore_artist|{name}|{artist}")),
                        InlineKeyboardButton(name2.upper(), callback_data=safe_callback(f"explore_artist|{name2}|{artist}")),
                    ])
                    i += 2
                    continue
        ...
        if rest:
            buttons.append([InlineKeyboardButton(
                f"+ {len(rest)} more artists…",
                callback_data=safe_callback(f"similar_more|{artist}")
            )])
    ...
    # ── similar_more: show remaining similar artists ──────────────────────────
    elif action == "similar_more":
        artist  = value
        similar = map_memory.get(chat_id, {}).get("similar", [])
        rest    = sorted(similar[12:], key=lambda x: len(x))
        buttons = []
        i = 0
        while i < len(rest):
            name = rest[i]
            if i + 1 < len(rest) and len(name) + len(rest[i+1]) <= 28:
                buttons.append([
                    InlineKeyboardButton(name.upper(),       callback_data=safe_callback(f"explore_artist|{name}|{artist}")),
                    InlineKeyboardButton(rest[i+1].upper(), callback_data=safe_callback(f"explore_artist|{rest[i+1]}|{artist}")),
                ])
                i += 2
            else:
                buttons.append([InlineKeyboardButton(name.upper(), callback_data=safe_callback(f"explore_artist|{name}|{artist}"))])
                i += 1
        buttons.append([InlineKeyboardButton(f"← Back to {artist[:20]}", callback_data=safe_callback(f"map_similar|{artist}"))])
        query.edit_message_text(
            f"🧬 Similar Artists — {artist} (all)",
            reply_markup=InlineKeyboardMarkup(buttons)
        )
