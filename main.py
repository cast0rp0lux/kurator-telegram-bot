...
def _build_genre_playlist(message, chat_id, style):
    ...
    """
    # Get top 500 artists for tag from Last.fm
    data  = lastfm("tag.gettopartists", tag=style, limit=500)
    items = data.get("topartists", {}).get("artist", [])
    names = [a.get("name", "") for a in items if a.get("name")]
    ...
    # Filter out mainstream artists — keep < 500K listeners for discovery
    obscure = []
    mainstream = []
    with ThreadPoolExecutor(max_workers=10) as ex:
        futures = {ex.submit(_fetch_listeners, a): a for a in names[:200]}
        for f in as_completed(futures):
            try:
                artist, listeners = f.result()
                if 0 < listeners < 500_000:
                    obscure.append(artist)
                elif listeners >= 500_000:
                    mainstream.append(artist)
            except Exception:
                pass

    # Mix: 70% obscure + 30% mainstream for balance
