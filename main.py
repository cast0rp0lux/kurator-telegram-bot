...
# ─── Version ──────────────────────────────────────────────────────────────────
BOT_VERSION = "Kurator 📀 Music Discovery Engine (v4.8.8)"
...
def _clean_track_title(title):
    """Remove all parenthetical content and noise for cleaner Soundiiz/Qobuz matching."""
    import re
    cleaned = title
...
def select_tracks_with_decades(artists, size=None, decades=None, message=None, mode="playlist", style_filter=None, use_mb=False, genre=None):
    ...
    with ThreadPoolExecutor(max_workers=10) as ex:
        futures = [ex.submit(_fetch_top_track, a) for a in filtered_pool[:target * 7]]
        for f in as_completed(futures):
            if len(tracks) >= target:
                break
            try:
...
def handle_buttons(update, context):
    ...
    if action == "cmd":
        ...
        if value == "menu":
            ...
            query.edit_message_text(
                "🗺️ Search an artist\n\n"
                "Type the name of any artist you want to explore:\n\n"
                "/map The Who\n"
                "/map The Specials\n"
                "/map Metallica\n\n"
                "Kurator will show you their world — styles, similar artists, albums and more.",
                reply_markup=InlineKeyboardMarkup([
                    [InlineKeyboardButton("← Back", callback_data="cmd|explore_menu")]
                ])
            )
            ...
            query.edit_message_text(
                "🎸 Play a genre\n\n"
                "Type any genre to get a playlist:\n\n"
                "/playlist post-punk\n"
                "/playlist jazz\n"
                "/playlist krautrock\n\n"
                "💡 For broad genres like rock or jazz, Kurator will ask you to pick a decade to keep things focused.",
                reply_markup=InlineKeyboardMarkup([
                    [InlineKeyboardButton("🏷️ Browse my tag library", callback_data="cmd|tags")],
                    [InlineKeyboardButton("← Back", callback_data="cmd|explore_menu")],
                ])
            )
