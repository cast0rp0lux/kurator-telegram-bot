...
# ─── Version ──────────────────────────────────────────────────────────────────
BOT_VERSION = "Kurator 📀 Music Discovery Engine (v4.7.8)"
...
def select_tracks(artists, size=None):
    ...
    keys_added = set()
    artists_used = []
    # Filter blacklisted and recently used artists
    artists = [a for a in artists
               if not _is_blacklisted(a)
               and normalize(a) not in {normalize(r) for r in _recent_artists}]
...
def playlist(update, context):
    ...
    if context.args:
        ...
        msg.reply_text(f"🎵🏷️ Building {tag.upper()} playlist…")
...
def handle_buttons(update, context):
    ...
    # ── decade_confirm: execute pending generation ────────────────────────────
        ...
        if gen_action == "playlist":
            ...
            # Trail always uses similar artists — era ignored
            if hops == 1:
