...
# ─── Version ──────────────────────────────────────────────────────────────────
BOT_VERSION = "Kurator 📀 Music Discovery Engine (v4.5.3)"
...
def _artist_active_in_decades(artist, decades):
    """
    Filter C — check if artist was active during any selected decade via MusicBrainz.
    Returns True if active, False if not, None if unknown (pass through).
    """
    try:
        mbid, _ = _mb_find_artist(artist)
        if not mbid:
            return None  # unknown — pass through
        mb = _mb_get(f"artist/{mbid}", {"fmt": "json"})
        ls    = mb.get("life-span", {})
        begin = ls.get("begin", "")
        end   = ls.get("end", "") or ""
        begin_year = int(begin[:4]) if begin and begin[:4].isdigit() else None
        end_year   = int(end[:4])   if end   and end[:4].isdigit()   else 9999
        ...
        for d in decades:
            ...
            # Active if began before end of decade AND ended after start of decade
...
def _artist_has_discogs_releases_in_decades(artist, decades):
    """
    Filter D — check if artist has Discogs releases in any selected decade.
    Returns True if yes, False if no, None if unknown.
    """
    try:
        r = requests.get(
            "https://api.discogs.com/database/search",
            params={"artist": artist, "type": "release", "per_page": 50, "token": DISCOGS_TOKEN},
            timeout=10
        ).json()
...
def _filter_artists_by_decade(artists, decades, message=None):
    """
    Apply C+D filter to a list of artists.
    Returns filtered list. Falls back to original list if too few pass.
    """
    if not decades:
        return artists

    # Filter C — active during decade (fast, parallel)
    if message:
    ...
    c_unknown = []
    with ThreadPoolExecutor(max_workers=10) as ex:
        futures = {ex.submit(_artist_active_in_decades, a, decades): a for a in artists}
        for f in as_completed(futures):
            artist = futures[f]
            try:
                result = f.result()
                if result is True:
                    c_passed.append(artist)
                elif result is None:
                    c_unknown.append(artist)
                # False = filtered out
            except Exception:
                c_unknown.append(artist)

    # Filter D — Discogs releases in decade (slower, only on C-passed + some unknowns)
    if message:
        try: message.reply_text("🎵 Checking releases…")
        except Exception: pass

    d_pool = c_passed + c_unknown[:min(len(c_unknown), 50)]
    d_passed = []
    with ThreadPoolExecutor(max_workers=8) as ex:
        futures = {ex.submit(_artist_has_discogs_releases_in_decades, a, decades): a for a in d_pool}
        for f in as_completed(futures):
            artist = futures[f]
            try:
                result = f.result()
                if result is True or result is None:
                    d_passed.append(artist)
            except Exception:
                d_passed.append(artist)

    log.info(f"Era filter: {len(artists)} → C:{len(c_passed)} → D:{len(d_passed)}")

    # Fallback if too few artists
    if len(d_passed) < 15:
        log.info("Era filter too restrictive, falling back to C-only")
        fallback = c_passed + c_unknown
        return fallback if len(fallback) >= 10 else artists

    return d_passed

def select_tracks_with_decades(artists, size=None, decades=None, message=None):
    """Select tracks from era-filtered artists using C+D algorithm."""
    if not decades:
...
def playlist(update, context):
    ...
    if context.args:
        ...
        send_playlist(msg, select_tracks(names, size=GENRE_PLAYLIST_SIZE),
                      title=f"🔖 {tag.upper()}", branded=False, chat_id=chat_id, size=GENRE_PLAYLIST_SIZE)
    else:
        ...
        send_playlist(msg, result, title="✦ Kurator's Playlist", branded=True, chat_id=chat_id)
