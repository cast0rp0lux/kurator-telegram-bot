...
# ─── Version ──────────────────────────────────────────────────────────────────
BOT_VERSION = "Kurator 📀 Music Discovery Engine (v4.8.9)"
...
TRACK_LINKS_PAGE    = 10
MB_USER_AGENT       = "Kurator/4.8.5 (telegram bot)"
...
def normalize_artist(name):
    """Normalize artist name for deduplication — strips common suffixes like '& The X'."""
    import re
    n = name.lower().strip()
...
def _fetch_track_from_early_albums(artist, decade_years=None):
    ...
    - Gets studio albums ordered by year from MusicBrainz
    - Picks from the first 3 albums (or albums within decade_years if specified)
    - For obscure artists (<100K listeners) disables top-track skip filter
    - Falls back to _fetch_top_track if no MB data or no tracks found
    decade_years: tuple (year_lo, year_hi) to restrict album selection by era
    """
    try:
        # Step 1: find MBID
        ...
        # Step 3: filter by era if specified, else take first 3
        if decade_years:
            lo, hi = decade_years
            era_albums = [a for a in albums if a.get("year") and lo <= int(a["year"]) <= hi]
            candidate_albums = era_albums[:4] if era_albums else albums[:3]
        else:
            candidate_albums = albums[:3]
...
def _resolve_genre_styles(genre, decades):
    """Return Discogs styles for a generic genre + set of decades."""
        ...
        try:
            ...
            if not active:
                ...
                if len(passed) >= TARGET:
                    break
            time.sleep(0.5)  # stay well within 1 req/s limit (2 calls per artist)
        except Exception:
            unknown.append(artist)
...
def _build_genre_playlist(message, chat_id, style):
    """
    Build a genre playlist without era — uses Last.fm tag.gettopartists
    filtered by listeners to surface obscure artists, not just mainstream ones.
    tag.gettopartists already includes listener counts — no extra API calls needed.
    """
    data  = lastfm("tag.gettopartists", tag=style, limit=1000)
    items = data.get("topartists", {}).get("artist", [])

    if not items:
        # Fallback to tag.gettoptracks
        ...
        send_playlist(message, result, title=f"🎵🏷️ {style.title()}",
                      branded=False, chat_id=chat_id, size=GENRE_PLAYLIST_SIZE,
                      map_chat_id=chat_id if chat_id in map_memory else None)
        return

    # Split into obscure (<500K listeners) and mainstream — listeners already in response
    obscure    = []
    mainstream = []
    unknown    = []
    for a in items:
        ...
        try:
            ...
            listeners = 0
        if 0 < listeners < 500_000:
            obscure.append(name)
        elif listeners >= 500_000:
            mainstream.append(name)
        else:
            unknown.append(name)  # listeners=0 or missing — include as fallback

    # 70% obscure + 30% mainstream for balance, unknown as last resort
    random.shuffle(obscure)
    random.shuffle(mainstream)
    random.shuffle(unknown)
    pool = obscure + mainstream[:max(5, GENRE_PLAYLIST_SIZE // 3)] + unknown[:20]
    log.info(f"Genre '{style}': {len(obscure)} obscure + {len(mainstream)} mainstream → pool {len(pool)}")

    result = select_tracks(pool, size=GENRE_PLAYLIST_SIZE, skip_recent=False)
    send_playlist(message, result, title=f"🎵🏷️ {style.title()}",
                  branded=False, chat_id=chat_id, size=GENRE_PLAYLIST_SIZE,
                  map_chat_id=chat_id if chat_id in map_memory else None)
...
def select_tracks_with_decades(artists, size=None, decades=None, message=None, mode="playlist", style_filter=None, use_mb=False, genre=None):
    ...
    # Resolve style_override from genre map if applicable
    style_override = None
    if genre and genre.lower() in GENRE_TO_DISCOGS_STYLES:
    ...
    if mode == "dig":
        ...
        # Filter by Last.fm listeners — ensures artist-level obscurity, not just release
        if message:
    ...
    # Optional MusicBrainz validation — sequential to respect 1 req/s rate limit
    if use_mb and verified and decades:
        if message:
            _safe_reply(message, "🔍 Verifying era…")
        mb_passed  = []
        mb_unknown = []
        for artist in verified:
            try:
                active = _artist_decade_from_mb(artist)
                if not active:
                    mb_unknown.append(artist)
                elif active & decades:
                    mb_passed.append(artist)
                time.sleep(0.5)
            except Exception:
                mb_unknown.append(artist)
        log.info(f"MB validation: {len(verified)} → {len(mb_passed)} passed, {len(mb_unknown)} unknown")
        if len(mb_passed) >= 10:
            verified = mb_passed
        elif len(mb_passed) + len(mb_unknown) >= 10:
            verified = mb_passed + mb_unknown[:max(0, 20 - len(mb_passed))]

    final_pool = verified if len(verified) >= 10 else pool

    # Compute decade year range for early album filtering
    ...
    # Select tracks with 1-artist-per-playlist rule and recent_artists filter
    global _recent_artists
    target     = size or PLAYLIST_SIZE
    ...
    if len(filtered_pool) < 10:
        filtered_pool = final_pool  # fallback if too many blocked
    ...
    with ThreadPoolExecutor(max_workers=8) as ex:
        futures = [ex.submit(_fetch_track_from_early_albums, a, decade_year_range)
                   for a in filtered_pool[:target * 7]]
        for f in as_completed(futures):
            if len(tracks) >= target:
                break
            try:
...
A real person's taste — not generated.

🎵 /playlist — A curated selection
⛏️ /dig — Deeper cuts from the same history
💎 /rare — Artists under 500K listeners only

All three let you filter by decade before generating.
Tap 🍌 GENERATE PLAYLIST to confirm.

🧭 FREE EXPLORE
───────────────
Navigate the music world freely.

🗺️ /map <artist> — Explore any artist's world
  Similar artists, styles, albums, trail.
  Example: /map The Who

🎸 /playlist <tag> — Genre playlists
  Example: /playlist post-punk
  For broad genres (rock, jazz...) Kurator will ask
  for a decade to keep things focused.

🏷️ /tags — Your personal genre library
  Built automatically as you explore the map.
  Tap any tag to generate a playlist.

⚙️ OTHER
────────
/status — Tracks in history, tags collected
/reset — Clear history and start fresh

🕰️ ERA FILTERING
────────────────
Decade filtering takes up to 60 seconds.
Kurator cross-checks artists against multiple sources.
...
def start(update, context):
    ...
    if is_onboarded(chat_id):
        update.message.reply_text(
            f"{BOT_VERSION}\n\n✦ Kurator's Picks — Playlists from a real listening history.\n🧭 Free Explore — Navigate the music map freely.",
...
def playlist(update, context):
    ...
    if context.args:
        ...
        if tag.lower().strip() in GENRES_NEED_ERA:
            # Generic genre — era is mandatory
            msg.reply_text(
                f"🎵🏷️ {tag.title()}\n\nThis is a broad genre — select a decade to get a focused playlist.",
                reply_markup=InlineKeyboardMarkup([
                    [InlineKeyboardButton("📅 Select a decade", callback_data="decade_open")],
            ...
            # Specific genre — show confirmation first
            msg.reply_text(
                f"🎵🏷️ {tag.title()}\n\nReady to generate?",
                reply_markup=InlineKeyboardMarkup([
                    [InlineKeyboardButton("🍌 GENERATE PLAYLIST", callback_data="decade_confirm")],
...
def handle_buttons(update, context):
    ...
    if action == "cmd":
        ...
        if value == "menu":
            query.edit_message_text(f"{BOT_VERSION}\n\n✦ Kurator's Picks — Playlists from a real listening history.\n🧭 Free Explore — Navigate the music map freely.", reply_markup=main_menu_markup())
            ...
            query.edit_message_text(
                ...
                reply_markup=InlineKeyboardMarkup([
                    [InlineKeyboardButton("🗺️ Search an artist",  callback_data="cmd|map_prompt")],
                    [InlineKeyboardButton("🎸 Play a genre",       callback_data="cmd|genre_prompt")],
                    [InlineKeyboardButton("🏷️ My tag library",    callback_data="cmd|tags")],
                    [InlineKeyboardButton("← Back", callback_data="cmd|menu")],
                ])
            )
            ...
            query.message.reply_text(
                "🎸 Play a genre\n\nReply with a genre name:\n\npost-punk · jazz · krautrock · shoegaze",
                reply_markup=ForceReply(selective=True)
            )
    ...
    # ── decade_confirm: execute pending generation ────────────────────────────
        ...
        if gen_action == "playlist":
            ...
            # If genre needs era and none selected — force era selector
            if style_lower in GENRES_NEED_ERA and not decades:
                _pending_gen[chat_id] = {"action": gen_action, "back": "cmd|menu"}
                query.edit_message_text(
                    f"🎵🏷️ {style.title()}\n\n"
                    f"This is a broad genre — select a decade to get a focused playlist.",
                    reply_markup=InlineKeyboardMarkup([
            ...
            if decades:
                # Pass genre for targeted Discogs style lookup
                genre_arg = style if style_lower in GENRES_NEED_ERA else None
                result = select_tracks_with_decades([], size=GENRE_PLAYLIST_SIZE,
                                                    decades=decades, message=message,
                                                    mode="playlist", use_mb=True,
                                                    genre=genre_arg)
                _cancel_working(sent, timer)
                era_tag  = f" — {_decade_label_from_set(decades)}" if decades else ""
                ...
                # No era — use _build_genre_playlist (same as button path)
                _cancel_working(sent, timer)
                _build_genre_playlist(message, chat_id, style)
    ...
    # ── build: playlist from style ────────────────────────────────────────────
        ...
        style = value
        _pending_decades.pop(chat_id, None)
        if style.lower().strip() in GENRES_NEED_ERA:
            # Generic genre — era mandatory
            ...
            # Specific genre — generate directly, no era needed
            _pending_gen[chat_id] = {"action": f"build|{style}", "back": f"map_back|{chat_id}"}
            query.edit_message_text(f"🎵🏷️ Building {style.title()} playlist…")
            sent, timer = _working_message(message, "Still building…", delay=50)
            _build_genre_playlist(message, chat_id, style)
    ...
    if action == "awaiting_map":
        ...
        if prev_id:
            try:
                ...
                pass
        mapping_msg = msg.reply_text(f"🔭 Mapping {text.upper()}…")
        _pending_map_msgs[chat_id] = mapping_msg.message_id
        sent, timer = _working_message(msg, "Still mapping…")
        _nav_history.pop(chat_id, None)
