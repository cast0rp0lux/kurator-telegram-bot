...
def _get_era_tracks_from_discogs(decades, style=None, mode="playlist", max_tracks=200):
    ...
    if style:
        styles = [style]
        years_per_style = 10  # more years when style is specific
    else:
        styles = []
        for d in decades:
...
def _get_style_tracks_from_discogs(style, mode="playlist", max_tracks=300):
    ...
    def _search_one(page):
        try:
            ...
            log.error(f"Discogs style search {style}: {e}")
            return []

    # Search across multiple pages for variety
    pages = random.sample(range(1, 10), 6)
    release_ids = []
    with ThreadPoolExecutor(max_workers=6) as ex:
        futures = [ex.submit(_search_one, p) for p in pages]
        for f in as_completed(futures):
            release_ids.extend(f.result())
