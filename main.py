...
# ─── Version ──────────────────────────────────────────────────────────────────
BOT_VERSION = "Kurator 📀 Music Discovery Engine (v3.8.1)"
...
# ─── Persistent history — timestamps + expiry ─────────────────────────────────
...
def _get_artist_info(artist_query):
    info = {
        ...
        "label": None,  # main record label
    }

    # ── MusicBrainz: two-step lookup ─────────────────────────────────────────
    try:
        r = requests.get(
            "https://musicbrainz.org/ws/2/artist/",
            params={"query": f'artist:"{artist_query}"', "fmt": "json", "limit": 5},
            headers={"User-Agent": "Kurator/3.8.1 (telegram bot)"},
            timeout=10
        )
        mbid = None
        if r.status_code == 200:
        ...
        if mbid:
            ...
            if r2.status_code == 200:
                ...
                log.info(f"MusicBrainz: '{mb.get('name')}' country={code} genres={info['genres']}")
    except Exception as e:
    ...
    try:
        ...
        if raw_bio:
            ...
            if len(clean) > 30:
                info["bio"] = clean
        # Fill genres from Last.fm if MusicBrainz returned none
        if not info["genres"]:
...
def _format_artist_card(artist_query, info):
    """Editorial artist card — uppercase name, country, years, label, genres."""
    lines = [artist_query.upper()]
    meta  = []
...
def _export_buttons(key):
    return [
        [InlineKeyboardButton("📤 Export via Soundiiz", url="https://soundiiz.com")],
        [InlineKeyboardButton("🔗 Open track links",    callback_data=f"sp_expand|{key}|0")],
...
def dig(update, context):
    ...
    send_playlist(msg, select_tracks(expand_artist_graph_deep(seeds)),
                  title="✦ Kurator's Dig", branded=True, chat_id=update.effective_chat.id)

def rare(update, context):
    msg = update.message
    msg.reply_text("💎 Searching for hidden gems…")
    send_playlist(msg, select_tracks(expand_artist_graph_rare(extract_seed_artists())),
                  title="✦ Kurator's Rare", branded=True, chat_id=update.effective_chat.id)
...
def _render_scene(message, artist_query, chat_id):
    ...
    for rel in data.get("results", []):
        ...
        for lbl in rel.get("label", []):
            if lbl and lbl.lower() not in ("not on label", "unknown"):
    ...
    buttons   = _build_scene_buttons(artist_query, sorted_styles, info, chat_id)
...
def _build_scene_buttons(artist_query, sorted_styles, info, chat_id):
    buttons = []
    ...
    if info.get("bio"):
        buttons.append([InlineKeyboardButton(
            "ℹ️ Artist Bio",
            callback_data=safe_callback(f"artist_bio|{artist_query}")
        )])

    # Label buttons — two modes
    label = info.get("label")
    if label:
        label_short = label[:20]
        buttons.append([
            InlineKeyboardButton(
                f"🏷 {label_short} — Hidden",
                callback_data=safe_callback(f"label_play|rare|{label}")
            ),
            InlineKeyboardButton(
                f"All",
                callback_data=safe_callback(f"label_play|all|{label}")
            ),
        ])

    # Genre pills — clickable, 2 per row
    if info.get("genres"):
        row = []
    ...
    for style, count in sorted_styles[:8]:
        buttons.append([InlineKeyboardButton(
            f"{style}  ({count})",
            callback_data=safe_callback(f"scene_style|{style}")
        )])
    if len(sorted_styles) > 8:
        buttons.append([InlineKeyboardButton(
            f"+ {len(sorted_styles)-8} more styles…",
            callback_data=f"scene_more|{chat_id}"
        )])
...
def _build_tags_buttons(sorted_tags, page, edit_mode=False):
    ...
    if edit_mode:
        ...
        row = []
        for tag, count in page_tags:
            row.append(InlineKeyboardButton(f"{tag} ({count})", callback_data=safe_callback(f"scene_style|{tag}")))
            if len(row) == 2:
                buttons.append(row)
                row = []
        if row:
...
def _render_status(message):
    ...
    expiry    = f"  Oldest track expires in {days_left} days\n" if days_left is not None else ""
    message.reply_text(
        f"📊 Status\n\n"
...
def handle_buttons(update, context):
    ...
    if action == "cmd":
        ...
        if value == "menu":
            ...
            send_playlist(message, select_tracks(expand_artist_graph_rare(extract_seed_artists())),
    ...
    # ── onboarding ────────────────────────────────────────────────────────────
        ...
        if step == 2:
            query.edit_message_text(
                ...
                reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("→ How do I start?", callback_data="onboard|3")]])
            )
        elif step == 3:
    ...
    # ── trail ─────────────────────────────────────────────────────────────────
        ...
        send_playlist(message, select_tracks(_expand_trail(artist, hops)),
                      title=f"🧬 {artist} — {labels.get(hops, '')}", branded=False, chat_id=chat_id)
    ...
    # ── scene_more ────────────────────────────────────────────────────────────
        ...
        buttons = [
            [InlineKeyboardButton(f"{s}  ({c})", callback_data=safe_callback(f"scene_style|{s}"))]
            for s, c in styles[8:]
        ]
        buttons.append([InlineKeyboardButton(f"⬅ Back to {artist[:20]}", callback_data=f"scene_back|{chat_id}")])
        query.edit_message_text(f"🌐 {artist} — all styles:", reply_markup=InlineKeyboardMarkup(buttons))

    # ── label_play: artists from a record label ───────────────────────────────
    elif action == "label_play":
        sub   = value.split("|", 1)
        mode  = sub[0]           # "rare" or "all"
        label = sub[1] if len(sub) > 1 else ""
        query.edit_message_text(f"🏷 Building {label} playlist…")
        try:
            r = requests.get(
                "https://api.discogs.com/database/search",
                params={"label": label, "type": "artist", "per_page": 100, "token": DISCOGS_TOKEN},
                timeout=15
            ).json()
            names = list({res["title"] for res in r.get("results", []) if res.get("title")})
        except Exception as e:
            log.error(f"Discogs label error: {e}")
            names = []
        if not names:
            query.edit_message_text(
                f"No artists found for label \"{label}\".",
                reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("⬅ Back", callback_data=f"scene_back|{chat_id}")]])
            )
            return
        if mode == "rare":
            # Filter by listeners
            filtered = []
            with ThreadPoolExecutor(max_workers=10) as ex:
                futures = [ex.submit(_fetch_listeners, a) for a in names[:100]]
                for f in as_completed(futures):
                    try:
                        artist, listeners = f.result()
                        if 0 < listeners < RARE_MAX_LISTENERS:
            ...
            names = filtered or names  # fallback if all filtered out
        send_playlist(message, select_tracks(names, size=GENRE_PLAYLIST_SIZE),
                      title=f"🏷 {label}{' — Hidden' if mode == 'rare' else ''}",
    ...
    # ── scene_style ───────────────────────────────────────────────────────────
        ...
        mem    = scene_memory.get(chat_id, {})
        artist = mem.get("artist", "")
        back   = f"⬅ Back to {artist[:20]}" if artist else "⬅ Back"
        query.edit_message_text(
            f"🌐 {value}\n\nGenerate a playlist for this style?",
            reply_markup=InlineKeyboardMarkup([
                [InlineKeyboardButton("✅ Generate playlist", callback_data=f"build|{value}")],
                [InlineKeyboardButton(back,                   callback_data=f"scene_back|{chat_id}")],
                [InlineKeyboardButton("⬅ Main menu",          callback_data="cmd|menu")],
            ])
        )
    ...
    # ── scene_back ────────────────────────────────────────────────────────────
        ...
        buttons   = _build_scene_buttons(artist, styles, info, chat_id)
    ...
    # ── sp_expand: paginated track links ─────────────────────────────────────
