...
# ─── Version ──────────────────────────────────────────────────────────────────
BOT_VERSION = "Kurator 📀 Music Discovery Engine (v4.0.1)"
...
def _format_artist_card(artist_query, info):
    """Build the artist card text — name, location, years, genres, label."""
    name  = _artist_display_name(info, artist_query)
    lines = [name, ""]
    meta  = []
    flag  = info.get("flag", "")
    if flag and info.get("country_name"):
        meta.append(f"{flag} {info['country_name']}")
    ...
    # Genres as plain text (not clickable in card)
    if info.get("genres"):
        lines.append("🎵 " + "  ·  ".join(info["genres"][:4]))
    if info.get("label"):
...
def send_playlist(message, tracks, title="✦ Kurator's Pick", branded=True, chat_id=None, size=None, map_chat_id=None):
    target = size or PLAYLIST_SIZE
    if not tracks:
        message.reply_text(
...
def handle_buttons(update, context):
    ...
    if action == "cmd":
        ...
        if value == "menu":
            ...
            result       = select_tracks(expand_artist_graph(seeds))
            _cancel_working(sent, timer)
            send_playlist(message, result, title="✦ Kurator's Pick", branded=True, chat_id=chat_id)
            ...
            result       = select_tracks(expand_artist_graph_deep(seeds))
            _cancel_working(sent, timer)
            send_playlist(message, result, title="✦ Kurator's Dig", branded=True, chat_id=chat_id)
            ...
            result       = select_tracks(expand_artist_graph_rare(extract_seed_artists()))
            _cancel_working(sent, timer)
            send_playlist(message, result, title="✦ Kurator's Rare", branded=True, chat_id=chat_id)
    ...
    # ── album_playlist ────────────────────────────────────────────────────────
        ...
        if not track_strings:
            ...
            return
        send_playlist(message, track_strings, title=f"💿 {album.upper()}", branded=False, chat_id=chat_id, map_chat_id=chat_id)
    ...
    # ── soundiiz_help ─────────────────────────────────────────────────────────
        ...
        message.reply_text(
            ...
            reply_markup=InlineKeyboardMarkup([
                [InlineKeyboardButton("🌐 Open Soundiiz", url="https://soundiiz.com")],
    ...
    # ── build: playlist from style ────────────────────────────────────────────
        ...
        random.shuffle(names)
        send_playlist(message, select_tracks(names, size=GENRE_PLAYLIST_SIZE),
                      title=f"🔍 {value}", branded=False, chat_id=chat_id,
                      size=GENRE_PLAYLIST_SIZE, map_chat_id=chat_id)
