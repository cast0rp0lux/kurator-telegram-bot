...
# ─── Version ──────────────────────────────────────────────────────────────────
BOT_VERSION = "Kurator 📀 Music Discovery Engine (v3.7.4)"
...
def get_spotify_token(chat_id):
    ...
    if not data:
        return None
    # Always try to refresh if we have a refresh token — ensures fresh product info
...
def _get_artist_info(artist_query):
    ...
    try:
        r = requests.get(
            ...
            headers={"User-Agent": "Kurator/3.7.4 (telegram bot)"},
            timeout=10
        )
        if r.status_code == 200:
            ...
            # Priority 1: exact name match (case-insensitive) with score >= 80
            for candidate in artists:
                score = int(candidate.get("score", 0))
                name  = candidate.get("name", "").lower()
                if score >= 80 and name == artist_query.lower():
                    mb = candidate
                    break

            # Priority 2: top result if score >= 90, regardless of exact name
            if not mb and artists and int(artists[0].get("score", 0)) >= 90:
                mb = artists[0]

            if mb:
                # Only use direct 'country' field — area codes are unreliable
                code = mb.get("country")
                if code:
                ...
                log.info(f"MusicBrainz: '{mb.get('name')}' score={mb.get('score')} country={code}")
            else:
                log.info(f"MusicBrainz: no confident match for '{artist_query}'")
    except Exception as e:
...
def reconnect_spotify(update, context):
    """Force fresh Spotify auth — use after changing subscription plan."""
    chat_id = str(update.effective_chat.id)
    if chat_id in _spotify_tokens:
        del _spotify_tokens[chat_id]
        save_spotify_tokens()
    # Trigger connect flow immediately
...
dp.add_handler(CommandHandler("reconnect",   reconnect_spotify))
