...
def select_tracks_with_decades(artists, size=None, decades=None, message=None, mode="playlist", style_filter=None, use_mb=False, genre=None):
    ...
    if genre and len(pool) < 50:
        ...
        for related in _expand_genre(genre, "near"):
            ...
            pool.extend([a for a in extra if a not in pool])
            log.info(f"  +{len(extra)} from near '{related}' → pool now {len(pool)}")
    ...
    if genre and len(pool) < 30:
        ...
        for related in _expand_genre(genre, "mid"):
            ...
            extra  = _get_era_artists_from_discogs(decades, mode=mode,
                                                    ...
                                                    style_override=styles)
            pool.extend([a for a in extra if a not in pool])
            log.info(f"  +{len(extra)} from mid '{related}' → pool now {len(pool)}")

    if genre and len(pool) < 15:
        log.warning(f"Pool very small ({len(pool)}) — expanding to far relatives as last resort")
        for related in _expand_genre(genre, "far"):
            if len(pool) >= 30:
                break
            styles = _resolve_genre_styles(related, decades)
            extra  = _get_era_artists_from_discogs(decades, mode=mode,
                                                    ...
                                                    style_override=styles)
            pool.extend([a for a in extra if a not in pool])
            log.info(f"  +{len(extra)} from far '{related}' → pool now {len(pool)}")
