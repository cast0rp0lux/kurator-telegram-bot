...
def _get_era_artists_from_lastfm(genre, decades, max_artists=150):
    ...
    # Step 2: pick 5 random seeds from top 50 (representative pool)
    ...
    pool = list(expanded)
    random.shuffle(pool)
    log.info(f"Genre '{genre}' expanded pool: {len(pool)} artists from {len(seeds)} seeds")

    if not decades:
        return pool[:max_artists]

    # Step 4: filter by era via MB album years — early stop
    lo = min(DECADE_YEARS[d][0] for d in decades)
    hi = max(DECADE_YEARS[d][1] for d in decades)

    passed  = []
    unknown = []
    TARGET  = min(max_artists, 80)
    ...
    for artist in pool:
        ...
        try:
            ...
            # If MB found albums but none in era → discard (wrong era artist)
        except Exception:
            unknown.append(artist)

    log.info(f"Genre '{genre}' era filter: {len(passed)} passed for {decades}")

    # Supplement with unknowns only if needed
    if len(passed) < 20:
